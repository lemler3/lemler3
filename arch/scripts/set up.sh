## user and groups
sudo usermod $USER -G sys,log,network,floppy,scanner,power,rfkill,users,video,storage,optical,lp,audio,wheel,adm,users
## sudo gpasswd -a $USER vboxusers ## after installing virtualbox
...
yay -S bash-completion jdk8-openjdk intel-ucode alsa-utils libva-vdpau-driver libvdpau-va-gl usbutils man ffmpegthumbnailer libappindicator-gtk2 libappindicator-gtk3 xorg --noconfirm

## set up qt5 settings (notes) (need to make into bash script)
# 1) yay -S  qt5ct adwaita-qt # for gtk2 qt5-styleplugins gtk-engine-murrine
...
yay -S firefox discord steam minecraft-launcher osbuddy runelite obs-studio pamac-all timeshift thunderbird lutris deluge-gtk gufw runescape-launcher google-chrome cronie --noconfirm && sudo systemctl enable --now cronie && sudo systemctl enable --now ufw
